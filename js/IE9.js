...
(function(N, p) {
  ...
  y(p, "onmouseup", function() {
    var c = br.instances;
    for (var a in c)
      if (!c[a][0].contains(event.srcElement)) br.unregister(c[a])
  });
  var co = {
    "=": "%1==='%2'",
    "~=": "(' '+%1+' ').indexOf(' %2 ')!==-1",
    ...
    var w = function(r) {
      ...
      var n = function(f, d) {
        var c = f.currentStyle["ie7-box-sizing"] === "border-box",
          a = 0;
        if (E && !c) a += o(f) + k(f, "padding");
        else if (!E && c) a -= o(f) + k(f, "padding");
        return q(f, d) + a
      };
  ...
  (function() {
    ...
    function k(d, c) {
      ...
      if (!d.runtimeStyle.autoLeft && d.currentStyle.marginLeft === "auto" && d.currentStyle.right !== "auto") {
        ...
        if (A(d.offsetParent)) d.runtimeStyle.pixelLeft += a;
        else d.runtimeStyle.shiftLeft = a
      }
  ...
  function dc() {
    ...
    function j(d) {
      ...
      while (d != B) {
        ...
        if (a) {
          if (k.test(a)) c *= parseFloat(a);
          else if (X.test(a)) c *= (parseFloat(a) / 100);
          else if (l.test(a)) c *= (parseFloat(a) / 2);
          else {
            m.style.fontSize = a;
            return 1
          }
    ...
    var R = h.PseudoElement = u.extend({
      ...
      init: function() {
        ...
        for (var c = 0; c < this.match.length; c++) {
          ...
          if (!a[this.position]) a[this.position] = {
            cssText: ""
          };
    ...
    bQ = new M("indeterminate", function(c) {
      ...
      h.CSS.addEventHandler(c, "onpropertychange", function() {
        if (event.propertyName === "indeterminate") {
          if (c.indeterminate === true) bQ.register(a);
