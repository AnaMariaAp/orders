...
var lenum= ' ABCÇDEFGHIJKLMNÑOPQRSTUVWXYZabcçdefghijklmnñopqrstuvwxyzàáÀÁéèÈÉíìÍÌïÏóòÓÒúùÚÙüÜ1234567890.'
...
//alert("validar")
	...
	if (x.value.length >0 && x.value.length <20){
	...
	*/
	...
	//N° de la cuenta 2
	/*
	x = document.f.numCuentaEmp12;
	x.value = trim(x.value);
	if (x.value.length==0){
		alert("Ingrese el N° de la cuenta.");
		x.focus();
		return false;
	}*/
	...
	//alert(": "+k);
	if (k == x.length){
	alert("Seleccione un Tipo de Modificación.");
	...
	if (k == 1){
		...
		x.value = trim(x.value);
		if (x.value.length==0){
			alert("Ingrese el Código de Empresa.");
			x.focus();
			return false;
		}
		else
...
function numletras(sText){
	var res=true;
	var char;
	for (i = 0; i < sText.length && res == true; i++){
		char = sText.charAt(i); 
		if (lenum.indexOf(char) == -1){
			res = false;
			break;
		}
	}
...
*/		
			//alert(1);
			y = document.f.numCuentaEmp2;
			z = document.f.monedaCuentaEmp2;
			for(i=0; i < y.length; i++ ){
			y[i].value="";
			y[i].disabled=false;			
			z[i].selectedIndex = 0;
